...
def payload_args(override, init_images, mask, prompt, negative_prompt, steps, sampler_name, cfg_scale, width, height,
                 batch_size, batch_count, controlnet, refiner_model, switch_point, resize_mode=0, denoising_strength=0.75, mask_blur=0, inpainting_fill=3,
...
def re_img(cfg, ori_img, mask_img, occ_mask_img, dep_pth, obj_prompt, url):
...
def gene_refine_inpaint_img(cfg, img_init, mask, occ, dep_pth, inpaint_gene_path, obj_prompt, url):
    img_lst = re_img(cfg, img_init, mask, occ, dep_pth, obj_prompt, url)
