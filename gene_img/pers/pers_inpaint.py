...
def payload_args(override, init_images, mask, prompt, negative_prompt, steps, sampler_name, cfg_scale, width, height,
                 batch_size, batch_count, controlnet, refiner_model, switch_point, resize_mode=0, denoising_strength=0.75, mask_blur=0, inpainting_fill=1,
...
def re_img(cfg, ori_pth, mask_pth, dep_pth, obj_prompt, url):
    ...
    payload = payload_args(override_settings, encoded_image_ori, [encoded_image_mask, encoded_image_mask], prompt, negative_prompt, steps,
...
def gene_inpaint_pers_img(cfg, ori_pth, mask_pth, dep_pth, inpaint_gene_path, obj_prompt, url):
    img_lst = re_img(cfg, ori_pth, mask_pth, dep_pth, obj_prompt, url)
