...
import utils.cameras.utils.poses as pose_utils
from utils.cameras import camera_utils
from utils.cameras.rays import RayBundle
from utils.cameras.utils.tensor_dataclass import TensorDataclass
...
class Cameras(TensorDataclass):
    ...
    def __init__(
        ...
        self.fx = self._init_get_fc_xy(fx, "fx")
        # @dataclass's post_init will take care of broadcasting
        self.fy = self._init_get_fc_xy(fy, "fy")

        # cx cy calculation
        # @dataclass's post_init will take care of broadcasting
        self.cx = self._init_get_fc_xy(cx, "cx")
        # @dataclass's post_init will take care of broadcasting
        self.cy = self._init_get_fc_xy(cy, "cy")
    ...
    def _init_get_camera_type(
        ...
        camera_type: Union[
            TensorType["batch_cam_types":...,
                       1], TensorType["batch_cam_types":...], int, List[CameraType], CameraType
        ],
    ...
    def _init_get_height_width(
        ...
        if isinstance(h_w, int):
            ...
            assert not torch.is_floating_point(
                h_w), f"height and width tensor must be of type int, not: {h_w.dtype}"
        ...
        # assert torch.all(h_w == h_w.view(-1)[0]), "Batched cameras of different h, w will be allowed in the future."
            ...
            raise ValueError(
                "Height must be an int, tensor, or None, received: " + str(type(h_w)))
        return h_w
    ...
    def get_image_coords(
        ...
        if index is None:
            ...
            image_coords = torch.meshgrid(torch.arange(
                image_height), torch.arange(image_width), indexing="ij")
    ...
    def generate_rays(  # pylint: disable=too-many-statements
        ...
        assert isinstance(camera_indices, (torch.Tensor, int)
                          ), "camera_indices must be a tensor or int"
        assert coords is None or isinstance(
            coords, torch.Tensor), "coords must be a tensor or None"
        ...
        if isinstance(camera_indices, torch.Tensor) and isinstance(coords, torch.Tensor):
            ...
            assert camera_opt_to_camera is None or camera_opt_to_camera.shape[
                :-2] == num_rays_shape, errormsg
            assert distortion_params_delta is None or distortion_params_delta.shape[
                :-1] == num_rays_shape, errormsg

        # If zero dimensional, we need to unsqueeze to get a batch dimension and then squeeze later
        if not self.shape:
            cameras = self.reshape((1,))
            assert torch.all(
                torch.tensor(camera_indices == 0) if isinstance(
                    camera_indices, int) else camera_indices == 0
        ...
        if cameras.is_jagged and coords is None and (keep_shape is None or keep_shape is False):
            ...
            _coords = [cameras.get_image_coords(index=tuple(
                index)).reshape(-1, 2) for index in camera_indices]
        ...
        if coords is None:
            ...
            coords: torch.Tensor = cameras.get_image_coords(
                index=tuple(index))  # (h, w, 2)
            coords = coords.reshape(
                coords.shape[:2] + (1,) * len(camera_indices.shape[:-1]) + (2,))  # (h, w, 1..., 2)
            # (h, w, num_rays, 2)
        ...
        camera_indices = camera_indices.broadcast_to(
            coords.shape[:-1] + (len(cameras.shape),)).to(torch.long)
    ...
    def _generate_rays_from_coords(
        ...
        assert camera_opt_to_camera is None or camera_opt_to_camera.shape == num_rays_shape + \
            (3, 4)
        ...
        true_indices = [camera_indices[..., i]
                        for i in range(camera_indices.shape[-1])]
        ...
        # (num_rays,)
        fx, fy = self.fx[true_indices].squeeze(
            -1), self.fy[true_indices].squeeze(-1)
        # (num_rays,)
        cx, cy = self.cx[true_indices].squeeze(
            -1), self.cy[true_indices].squeeze(-1)
        assert (
        ...
        coord = torch.stack(
            [(x - cx) / fx, -(y - cy) / fy], -1)  # (num_rays, 2)
        coord_x_offset = torch.stack(
            [(x - cx + 1) / fx, -(y - cy) / fy], -1)  # (num_rays, 2)
        coord_y_offset = torch.stack(
            [(x - cx) / fx, -(y - cy + 1) / fy], -1)  # (num_rays, 2)
        assert (
        ...
        coord_stack = torch.stack(
            [coord, coord_x_offset, coord_y_offset], dim=0)  # (3, num_rays, 2)
        ...
        if not disable_distortion:
            ...
            if distortion_params is not None:
                # (num_rays)
                mask = (self.camera_type[true_indices] !=
                        CameraType.EQUIRECTANGULAR.value).squeeze(-1)
        ...
        if CameraType.FISHEYE.value in cam_types:
            ...
            directions_stack[..., 0][mask] = torch.masked_select(
                coord_stack[..., 0] * sin_theta / theta, mask).float()
            directions_stack[..., 1][mask] = torch.masked_select(
                coord_stack[..., 1] * sin_theta / theta, mask).float()
            directions_stack[..., 2][mask] = - \
                torch.masked_select(torch.cos(theta), mask).float()

        if CameraType.PANORAMA.value in cam_types:
            ...
            # (num_rays)
            mask = (self.camera_type[true_indices] ==
                    CameraType.PANORAMA.value).squeeze(-1)
            mask = torch.stack([mask, mask, mask], dim=0)
            ...
            # del xx, yy, zz
            directions_stack[..., 0][mask] = torch.masked_select(
                torch.sin(theta) * torch.cos(phi), mask).float()
            directions_stack[..., 1][mask] = torch.masked_select(
                torch.sin(phi), mask).float()
            directions_stack[..., 2][mask] = torch.masked_select(
                -torch.cos(theta) * torch.cos(phi), mask).float()
        ...
        if CameraType.EQUIRECTANGULAR.value in cam_types:
            ...
            directions_stack[..., 0][mask] = torch.masked_select(
                -torch.sin(theta) * torch.sin(phi), mask).float()
            directions_stack[..., 1][mask] = torch.masked_select(
                torch.cos(phi), mask).float()
            directions_stack[..., 2][mask] = torch.masked_select(
                -torch.cos(theta) * torch.sin(phi), mask).float()
        ...
        directions_stack, directions_norm = camera_utils.normalize_with_norm(
            directions_stack, -1)
        assert directions_stack.shape == (3,) + num_rays_shape + (3,)
        ...
        times = self.times[camera_indices,
                           0] if self.times is not None else None
        ...
        if image is not None:
            ...
            if max_size is not None:
                ...
                image_uint8 = torchvision.transforms.functional.resize(
                    image_uint8, max_size)  # type: ignore
                image_uint8 = image_uint8.permute(1, 2, 0)
            image_uint8 = image_uint8.cpu().numpy()
            data = cv2.imencode(".jpg", image_uint8)[1].tobytes()
            json_["image"] = str("data:image/jpeg;base64," +
    ...
    def rescale_output_resolution(
        ...
        if isinstance(scaling_factor, (float, int)):
            scaling_factor = torch.tensor([scaling_factor]).to(
                self.device).broadcast_to((self.cx.shape))
