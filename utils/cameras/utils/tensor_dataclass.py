...
class TensorDataclass:
    ...
    def __post_init__(self) -> None:
        ...
        broadcasted_fields = self._broadcast_dict_fields(
            {f.name: self.__getattribute__(
                f.name) for f in dataclasses.fields(self)}, batch_shape
        )
    ...
    def __getitem__(self: TensorDataclassT, indices) -> TensorDataclassT:
        ...
        def tensor_fn(x): return x[indices + (slice(None),)]
        def dataclass_fn(x): return x[indices]
    ...
    def reshape(self: TensorDataclassT, shape: Tuple[int, ...]) -> TensorDataclassT:
        ...
        def tensor_fn(x): return x.reshape((*shape, x.shape[-1]))
        def dataclass_fn(x): return x.reshape(shape)
